...
class ChessGame:
    ...
    def move_piece(self, start_pos, end_pos):
        ...
        king_pos = self._find_king(self.turn)
        if self._is_under_attack(king_pos[0], king_pos[1], 'Black' if self.turn == 'White' else 'White'):
            if self.is_checkmate(self.turn):
                print(f"체크메이트! {'Black' if self.turn == 'White' else 'White'} 승리!")
            else:
        ...
        for r in range(8):
            for c in range(8):
                ...
                if piece != ' ' and ((turn == 'White' and piece.isupper()) or (turn == 'Black' and piece.islower())):
                    start_pos = self.from_coords(r, c)
                    for r2 in range(8):
                        for c2 in range(8):
                            end_pos = self.from_coords(r2, c2)
                            if self.is_valid_move(start_pos, end_pos):
                                moves.append((start_pos, end_pos))
        return moves

    def is_checkmate(self, turn):
        return len(self.get_legal_moves(turn)) == 0 and \
               self._is_under_attack(*self._find_king(turn), 'Black' if turn == 'White' else 'White')

    def is_stalemate(self, turn):
        return len(self.get_legal_moves(turn)) == 0 and \
               not self._is_under_attack(*self._find_king(turn), 'Black' if turn == 'White' else 'White')
